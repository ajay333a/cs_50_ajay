...
def buy():
    ...
    if request.method == "GET":
        ...
        if shares < 0:
            return apology("Not allowed to buy negative stocks")
...
def sell():
    ...
    if request.method == "GET":
        ...
        user_shares = db.execute("SELECT shares from transactions WHERE id = :id AND symbol = :symbol GROUP BY symbol", id = user_id, symbol = symbol)
        user_shares_real = user_shares[0]["shares"]

        uptd_cash = user_cash + transaction_value

        if shares < user_shares_real:
            return apology("You Do Not have the required shares")
