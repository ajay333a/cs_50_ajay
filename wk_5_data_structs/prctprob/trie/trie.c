...
bool check(char *word)
{
    ...
    for (int i = 0, n = strlen(word); i < n; i++) //looping over alphabets in the name
    {
        int index = tolower(word[i]) - 'a'; //converting uppercase alphabets to lower case
        if (index < 0 || index >= SIZE_OF_ALPHABET) //checking if index is not exceeding above 26 or below 0
        {
            return false;
        }
        if (cursor -> children[index] == NULL)
